...
(define-values (_ bl cl)
  (apply values
   (fold-right
    (lambda (i sp-bl-cl)
      (let-values ([(sp bl cl) (apply values sp-bl-cl)])
        (let ([b-w (inexact (get-random-value 50 200))]
              [b-h (inexact (get-random-value 100 800))])
          (list (+ sp b-w)
                (cons (list (- sp 6000.0) (- screen-height b-h 130.0) b-w b-h) bl)
                (cons (list (get-random-value 200 240)
...
(define camera
  (let ([c (make-camera-2d 0.0 0.0 0.0 1.0)])
    (vector-2-set! (camera-2d-ref& c offset) x (float (/ screen-width 2.0)))
    (vector-2-set! (camera-2d-ref& c offset) y (float (/ screen-height 2.0)))
...
(do ([player-x 400 (cond [(is-key-down KEY_RIGHT) (+ player-x 2)]
                          [(is-key-down KEY_LEFT) (- player-x 2)]
                          [else player-x])]
     [player-y 280])
    ((window-should-close) (close-window))
  (camera-2d-set! camera rotation
    (min (max (+ (camera-2d-get camera rotation)
                 (cond [(is-key-down KEY_A) -1.0]
                       [(is-key-down KEY_S) 1.0]
                       [else 0])) -40.0) 40.0))
  (camera-2d-set! camera zoom
    (min (max (+ (camera-2d-get camera zoom)
                 (* 0.05 (get-mouse-wheel-move))) 0.1) 3.0))
  (when (is-key-pressed KEY_R)
    (camera-2d-set! camera zoom 1.0)
    (camera-2d-set! camera rotation 0.0))
  (drawing-begin
   (clear-background RAYWHITE)
   (mode-2d-begin camera
     (draw-rectangle -6000 320 13000 8000 DARKGRAY)
     (vector-for-each (lambda (building color) (draw-rectangle-rec building color))
                      buildings build-colors)
     (draw-rectangle player-x player-y 40 40 RED)
     (draw-line (int (camera-2d-get camera (target x))) (* screen-height -10)
                (int (camera-2d-get camera (target x))) (* screen-height 10) GREEN)
     (draw-line (* screen-width -10) (int (camera-2d-get camera (target y)))
                (* screen-width 10) (int (camera-2d-get camera (target y))) GREEN))
