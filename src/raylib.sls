(library (raylib raylib (0 3))
  (export rad->deg deg->rad PI make-color drawing-begin
   ...
   LIGHTGRAY MakeAudio-Callback MakeSave-File-Text-Callback
   MakeLoad-File-Text-Callback MakeSave-File-Data-Callback
   MakeLoad-File-Data-Callback MakeTrace-Log-Callback
   detach-audio-mixed-processor attach-audio-mixed-processor
   detach-audio-stream-processor attach-audio-stream-processor
   ...
   draw-triangle-strip3-d draw-triangle3-d draw-circle3-d
   draw-point3-d draw-line3-d text-to-float text-to-integer
   ...
   load-automation-event-list compute-sha-2-5-6 compute-sha-1
   compute-md-5 compute-crc-3-2 decode-data-base64
   encode-data-base64 decompress-data compress-data
   ...
   set-target-fps get-camera-matrix2-d get-camera-matrix
   get-screen-to-world2-d get-world-to-screen2-d
   ...
   begin-texture-mode end-mode3-d begin-mode3-d end-mode2-d
   begin-mode2-d end-drawing begin-drawing clear-background
  ...
  (begin
          ...
          (unless f
            (set! f
              (foreign-procedure "CheckCollisionSpheres"
                ...
                unsigned-8)))
          (not (= (f center1 radius1 center2 radius2) 0)))))
          ...
          (unless f
            (set! f
              (foreign-procedure "DrawBillboard"
                ((& Camera-3-D) (& Texture) (& Vector-3) float (& Color))
                void)))
    ...
    (define color-lerp
      (let ([f #f])
        (lambda (color1 color2 factor)
          (unless f
            (set! f
              (foreign-procedure "ColorLerp"
                ((& Color) (& Color) float)
                (& Color))))
          (f color1 color2 factor))))
    ...
    (define image-draw-rectangle-gradient-ex
      (let ([f #f])
        (lambda (dst rec col1 col2 col3 col4)
          (unless f
            (set! f
            ...
            (set! f
              (foreign-procedure "ImageDrawTriangleLines"
                ((* Image)
                 ...
                 (& Color))
                void)))
          (f dst v1 v2 v3 color))))
    (define image-draw-triangle-gradient
      (let ([f #f])
        (lambda (dst v1 v2 v3 c1 c2 c3)
          (unless f
            (set! f
    ...
    (define gen-image-checked
      (let ([f #f])
        (lambda (width height checks-x checks-y col1 col2)
          (unless f
            (set! f
              (foreign-procedure "GenImageChecked"
                (int int int int (& Color) (& Color))
                (& Image))))
          (f width height checks-x checks-y col1 col2))))
    ...
    (define check-collision-lines
      (let ([f #f])
        (lambda (start-pos1 end-pos1 start-pos2 end-pos2
                 collision-point)
          (unless f
            (set! f
              (foreign-procedure "CheckCollisionLines"
                ((& Vector-2)
                 ...
                 (* Vector-2))
                unsigned-8)))
          (not (= (f start-pos1 end-pos1 start-pos2 end-pos2
                     collision-point)
                  0)))))
    ...
    (define draw-spline-segment-bezier-cubic
      (let ([f #f])
        (lambda (p1 c2 c3 p4 thick color)
          (unless f
            (set! f
          ...
          (unless f
            (set! f
              (foreign-procedure "UpdateCameraPro"
                ((* Camera-3-D) (& Vector-3) (& Vector-3) float)
                void)))
    ...
    (define begin-mode3-d
      (let ([f #f])
        (lambda (camera)
          (unless f
            (set! f
              (foreign-procedure "BeginMode3D" ((& Camera-3-D)) void)))
          (f camera))))
    (define end-mode2-d
      (let ([f #f])
        (lambda ()
    ...
    (define begin-mode2-d
      (let ([f #f])
        (lambda (camera)
          (unless f
            (set! f
              (foreign-procedure "BeginMode2D" ((& Camera-2-D)) void)))
          (f camera))))
    (define end-drawing
      (let ([f #f])
        (lambda ()
    ...
    (define MakeMatrix
      (case-lambda
        [(m-0 m-4 m-8 m-1-2 m-1 m-5 m-9 m-1-3 m-2 m-6 m-1-0 m-1-4
          m-3 m-7 m-1-1 m-1-5)
         (let ([s (make-ftype-pointer
                    Matrix
      ...
      (lambda (src)
        (let ([dst (make-ftype-pointer
          ...
          (ftype-set!
            ...
            (m-1-0)
            dst
            (ftype-ref Matrix (m-1-0) src))
          (ftype-set!
            Matrix
            (m-1-4)
            dst
            (ftype-ref Matrix (m-1-4) src))
          ...
          (ftype-set!
            ...
            (m-1-1)
            dst
            (ftype-ref Matrix (m-1-1) src))
          (ftype-set!
            Matrix
            (m-1-5)
            dst
            (ftype-ref Matrix (m-1-5) src))
          dst)))
      ...
      (case-lambda
        [(r g b a)
         (let ([s (make-ftype-pointer
                    Color
                    (foreign-alloc (ftype-sizeof Color)))])
           (ftype-set! Color (r) s r)
           (ftype-set! Color (g) s g)
           (ftype-set! Color (b) s b)
           (ftype-set! Color (a) s a)
           s)]
        [(struct r g b a)
         (ftype-set! Color (r) struct r)
         (ftype-set! Color (g) struct g)
    ...
    (define CopyColor
      (lambda (src)
        (let ([dst (make-ftype-pointer
                     Color
                     (foreign-alloc (ftype-sizeof Color)))])
          (ftype-set! Color (r) dst (ftype-ref Color (r) src))
          (ftype-set! Color (g) dst (ftype-ref Color (g) src))
          (ftype-set! Color (b) dst (ftype-ref Color (b) src))
          (ftype-set! Color (a) dst (ftype-ref Color (a) src))
          dst)))
      ...
      (case-lambda
        [(source left top right bottom layout)
         (let ([s (make-ftype-pointer
           ...
           (void)
           (ftype-set! NPatch-Info (left) s left)
           (ftype-set! NPatch-Info (top) s top)
           (ftype-set! NPatch-Info (right) s right)
           (ftype-set! NPatch-Info (bottom) s bottom)
           (ftype-set! NPatch-Info (layout) s layout)
           s)]
        [(struct source left top right bottom layout)
         (void)
         (ftype-set! NPatch-Info (left) struct left)
         (ftype-set! NPatch-Info (top) struct top)
         (ftype-set! NPatch-Info (right) struct right)
         (ftype-set! NPatch-Info (bottom) struct bottom)
         (ftype-set! NPatch-Info (layout) struct layout)
         struct]))
    (define CopyNPatch-Info
      (lambda (src)
        (let ([dst (make-ftype-pointer
                     NPatch-Info
                     (foreign-alloc (ftype-sizeof NPatch-Info)))])
          ...
          (ftype-set!
            ...
            (left)
            dst
            (ftype-ref NPatch-Info (left) src))
          (ftype-set!
            NPatch-Info
            (top)
            dst
            (ftype-ref NPatch-Info (top) src))
          (ftype-set!
            NPatch-Info
            (right)
            dst
            (ftype-ref NPatch-Info (right) src))
          (ftype-set!
            NPatch-Info
            (bottom)
            dst
            (ftype-ref NPatch-Info (bottom) src))
          (ftype-set!
            NPatch-Info
            (layout)
            dst
            (ftype-ref NPatch-Info (layout) src))
          dst)))
    ...
    (define-ftype Camera-3-D
      (struct
        ...
        [fovy float]
        [projection int]))
    (define MakeCamera-3-D
      (case-lambda
        [(position target up fovy projection)
         (let ([s (make-ftype-pointer
                    Camera-3-D
                    (foreign-alloc (ftype-sizeof Camera-3-D)))])
           (void)
           (void)
           (void)
           (ftype-set! Camera-3-D (fovy) s fovy)
           (ftype-set! Camera-3-D (projection) s projection)
           s)]
        [(struct position target up fovy projection)
         (void)
         (void)
         (void)
         (ftype-set! Camera-3-D (fovy) struct fovy)
         (ftype-set! Camera-3-D (projection) struct projection)
         struct]))
    (define CopyCamera-3-D
      (lambda (src)
        (let ([dst (make-ftype-pointer
                     Camera-3-D
                     (foreign-alloc (ftype-sizeof Camera-3-D)))])
          ...
          (ftype-set!
            ...
            (fovy)
            dst
            (ftype-ref Camera-3-D (fovy) src))
          (ftype-set!
            Camera-3-D
            (projection)
            dst
            (ftype-ref Camera-3-D (projection) src))
          dst)))
    (define-syntax Camera-3-D-set!
      (syntax-rules ()
        [(_ s (f ...) v) (ftype-set! Camera-3-D (f ...) s v)]
        [(_ s f v) (ftype-set! Camera-3-D (f) s v)]))
    (define-syntax Camera-3-D-get
      (syntax-rules ()
        [(_ s (f ...)) (ftype-ref Camera-3-D (f ...) s)]
        [(_ s f) (ftype-ref Camera-3-D (f) s)]))
    (define-syntax Camera-3-D-ref&
      (syntax-rules ()
        [(_ s (f ...)) (ftype-&ref Camera-3-D (f ...) s)]
        [(_ s f) (ftype-&ref Camera-3-D (f) s)]))
    (define-ftype Camera-2-D
      (struct
        [offset Vector-2]
        [target Vector-2]
        [rotation float]
        [zoom float]))
    (define MakeCamera-2-D
      (case-lambda
        [(offset target rotation zoom)
         (let ([s (make-ftype-pointer
                    Camera-2-D
                    (foreign-alloc (ftype-sizeof Camera-2-D)))])
           (void)
           (void)
           (ftype-set! Camera-2-D (rotation) s rotation)
           (ftype-set! Camera-2-D (zoom) s zoom)
           s)]
        [(struct offset target rotation zoom)
         (void)
         (void)
         (ftype-set! Camera-2-D (rotation) struct rotation)
         (ftype-set! Camera-2-D (zoom) struct zoom)
         struct]))
    (define CopyCamera-2-D
      (lambda (src)
        (let ([dst (make-ftype-pointer
                     Camera-2-D
                     (foreign-alloc (ftype-sizeof Camera-2-D)))])
          ...
          (ftype-set!
            ...
            (rotation)
            dst
            (ftype-ref Camera-2-D (rotation) src))
          (ftype-set!
            Camera-2-D
            (zoom)
            dst
            (ftype-ref Camera-2-D (zoom) src))
          dst)))
    (define-syntax Camera-2-D-set!
      (syntax-rules ()
        [(_ s (f ...) v) (ftype-set! Camera-2-D (f ...) s v)]
        [(_ s f v) (ftype-set! Camera-2-D (f) s v)]))
    (define-syntax Camera-2-D-get
      (syntax-rules ()
        [(_ s (f ...)) (ftype-ref Camera-2-D (f ...) s)]
        [(_ s f) (ftype-ref Camera-2-D (f) s)]))
    (define-syntax Camera-2-D-ref&
      (syntax-rules ()
        [(_ s (f ...)) (ftype-&ref Camera-2-D (f ...) s)]
        [(_ s f) (ftype-&ref Camera-2-D (f) s)]))
    (define-ftype Mesh
    ...
    (define MakeAutomation-Event-List
      (case-lambda
        ...
        [(struct capacity count events)
         ...
         (void)
         struct]))
    (define CopyAutomation-Event-List
      (lambda (src)
        (let ([dst (make-ftype-pointer
                     Automation-Event-List
                     (foreign-alloc
